...
namespace Silverback.Messaging.Subscribers
{
    internal class SubscribedMethodInvoker
    {
            ...
            switch (messageArgumentResolver)
            {
                ...
                default:
                    throw new SubscribedMethodInvocationException(
                        "The message argument resolver must implement either ISingleMessageArgumentResolver " +
                        $"or IEnumerableMessageArgumentResolver, the type '{messageArgumentResolver}' isn't " +
        ...
        private static IEnumerable<object> UnwrapEnvelopesIfNeeded(
            IEnumerable<object> messages,
            Type targetMessageType) =>
            typeof(IEnvelope).IsAssignableFrom(targetMessageType) ? messages : UnwrapEnvelopesIfAutoUnwrap(messages);

        private static IEnumerable<object> UnwrapEnvelopesIfAutoUnwrap(IEnumerable<object> messages)
        {
